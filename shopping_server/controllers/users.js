var { Email, Head } = require('../db/models');
var fs = require('fs');
var url = require('url');
var EmailUserModel = require('../db/emailUsers')
var { setCrypto , createVerify } = require('../util/base');
...
var login = async (req,res,next)=>{
  var { username , password, verifyImg } = req.body;
	...
	var result = await EmailUserModel.findLogin({
		username,
...
var register = async (req,res,next)=>{
  var { username , password , email , verify } = req.body;
	...
	var result = await EmailUserModel.save({
		username,
		password: setCrypto(password),
		email
	});
...
var verify = async (req,res,next)=>{
	...
	var mailOptions = {
	    from: '简易外卖App 3478162259@qq.com',
	    to: email,
	    subject: '简易外卖App邮箱验证码',
	    text: '来自简易外卖App的验证码：' + verify
	}
...
var findPassword = async (req,res,next)=>{
	...
	if( email === req.session.email && verify === req.session.verify ){
		var result = await EmailUserModel.updatePassword(email , setCrypto(password));
		if(result){
			res.send({
				msg : '修改密码成功',
				status : 0
...
var uploadUserHead = async (req,res,next)=>{
	...
	var result = await EmailUserModel.updateUserHead( req.session.username , url.resolve(Head.baseUrl , req.session.username + '.jpg' ) );
