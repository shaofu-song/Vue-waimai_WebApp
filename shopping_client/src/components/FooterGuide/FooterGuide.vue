<template>
  <div class="footer_guide">
    ...
    <a href="javascript:;" class="guide_item" :class="{on: '/profile'===$route.path}" 
      @click="goTo('/profile')" v-if="!this.$store.state.user.emailLoginStatus">
      <span class="item_icon">
        <i class="iconfont icon-geren"></i>
      </span>
      <span>我的</span>
    </a>
    <a href="javascript:;" class="guide_item" :class="{on: '/emailProfile'===$route.path}" 
      @click="goTo('/emailProfile')" v-else>
      <span class="item_icon">
        <i class="iconfont icon-geren"></i>
      </span>
      <span>我的</span>
...
export default {
  // computed: {
  //   ...mapState('user', [emailLoginStatus])
  // },
